...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (isset($_POST['archive_class']) && $user['role'] === 'teacher') {
        try {
            ...
            $stmt->execute([$class_id, $user['id']]);
            header('Location: ../home/dashboard.php'); exit;
        } catch (PDOException $e) { $errors[] = 'Error archiving class: ' . $e->getMessage(); }
        ...
        if (empty($message)) {
            ...
            try {
                ...
                header('Location: stream.php?class_id=' . $class_id); exit;
            } catch (PDOException $e) { $errors[] = 'Error posting announcement: ' . $e->getMessage(); }
        }
    }
}
...
<head>
    ...
    <style>
        .class-header {
            ...
            color: white; padding: 80px 24px 24px;
            border-radius: 12px; margin: 0 auto 24px; max-width: 1000px;
        }
        ...
        .stream-empty-title {
            font-size: 18px; font-weight: 600; color: #5f6368; margin-bottom: 8px;
        }
        ...
        .ann-avatar-circle { width: 40px; height: 40px; border-radius: 50%; background: #1a73e8; color: #fff; display: flex; align-items: center; justify-content: center; font-weight: 700; font-size: 15px; flex-shrink: 0; }
...
<body>
            ...
            <div class="stream-container">
                ...
                <div style="display:flex; gap:24px; align-items:flex-start;">
                    ...
                    <div style="flex:1;">
                        ...
                        <div class="ann-card" data-ann-id="<?php echo $ann['id']; ?>">
                            ...
                            <div class="ann-inline-comment-section" id="ann-inline-<?php echo $ann['id']; ?>" style="display:none;">
                                ...
                                <div class="ann-inline-comment-item">
                                    <div class="ann-avatar-circle" style="width:32px;height:32px;font-size:12px;flex-shrink:0;"><?php echo strtoupper(substr($c['commenter_name'], 0, 1)); ?></div>
                                ...
                                <div class="ann-inline-input-row">
                                    <div class="ann-avatar-circle" style="width:32px;height:32px;font-size:12px;flex-shrink:0;"><?php echo strtoupper(substr($user['name'], 0, 1)); ?></div>
                        ...
                        <?php endforeach; ?>
                        <?php endif; // end stream_empty check ?>
                    </div>
                </div>
            </div>
        </main>
    ...
    function closeAnnModal() {
        ...
        const ta = document.getElementById('annTextarea'); ta.value = ''; ta.classList.remove('has-text');
        const btn = document.getElementById('annPostBtn'); btn.disabled = true; btn.classList.remove('active');
    }
    ...
    function toggleAnnComment(id) { const s=document.getElementById('ann-inline-'+id), h=s.style.display==='none'; s.style.display=h?'block':'none'; if(h) setTimeout(()=>document.getElementById('ann-ctextarea-'+id).focus(),80); }
    function expandAnnComment(id) { document.getElementById('ann-pill-'+id).classList.add('expanded'); document.getElementById('ann-toolbar-'+id).style.display='flex'; }
    ...
    function submitAnnComment(annId) {
        const ta=document.getElementById('ann-ctextarea-'+annId), msg=ta.value.trim(); if(!msg) return;
        ...
        .then(r=>r.json()).then(data=>{
            if(data.success){
                ta.value=''; const btn=document.getElementById('ann-csend-'+annId),img=document.getElementById('ann-send-img-'+annId); btn.disabled=true; if(img) img.style.opacity='0.4';
                ...
                div.innerHTML=`<div class="ann-avatar-circle" style="width:32px;height:32px;font-size:12px;flex-shrink:0;"><?php echo strtoupper(substr($user['name'],0,1)); ?></div><div class="ann-inline-comment-body"><span class="ann-inline-comment-author"><?php echo htmlspecialchars($user['name']); ?></span><span class="ann-inline-comment-date">Just now</span><div class="ann-inline-comment-text">${msg}</div></div>`;
                section.insertBefore(div,inputRow);
                const countEl=document.getElementById('ann-count-'+annId); if(countEl) countEl.textContent=parseInt(countEl.textContent||0)+1;
            }
        });
    }
    document.addEventListener('input',function(e){
        if(e.target.classList.contains('ann-comment-pill-input')){const id=e.target.id.replace('ann-ctextarea-',''),btn=document.getElementById('ann-csend-'+id),img=document.getElementById('ann-send-img-'+id),hasVal=e.target.value.trim().length>0;btn.disabled=!hasVal;if(img)img.style.opacity=hasVal?'1':'0.4';}
        if(e.target.id==='asgnModalInput'){const hasVal=e.target.value.trim().length>0;document.getElementById('asgnModalSend').disabled=!hasVal;document.getElementById('asgnModalSendImg').style.opacity=hasVal?'1':'0.4';}
    });
    document.addEventListener('click',function(e){
        document.querySelectorAll('.ann-comment-pill-wrapper').forEach(wrapper=>{const id=wrapper.id.replace('ann-pill-',''),ta=document.getElementById('ann-ctextarea-'+id);if(!wrapper.contains(e.target)&&ta&&!ta.value.trim()){wrapper.classList.remove('expanded');const toolbar=document.getElementById('ann-toolbar-'+id);if(toolbar)toolbar.style.display='none';}});
    });
    let currentAsgnId=null;
    function openAsgnCommentModal(asgnId){
        currentAsgnId=asgnId; document.getElementById('asgnCommentOverlay').classList.add('open');
        document.getElementById('asgnModalBody').innerHTML='<p style="color:#5f6368;font-size:13px;">Loading...</p>';
        document.getElementById('asgnModalInput').value=''; document.getElementById('asgnModalSend').disabled=true; document.getElementById('asgnModalSendImg').style.opacity='0.4';
        fetch('get_comments.php?entity_id='+asgnId+'&entity_type=assignment').then(r=>r.json()).then(data=>{
            ...
            if(!data.comments||data.comments.length===0){body.innerHTML='<p style="color:#5f6368;font-size:13px;">No comments yet.</p>';}
            else{body.innerHTML='';data.comments.forEach(c=>{const div=document.createElement('div');div.className='ann-inline-comment-item';div.innerHTML=`<div class="ann-avatar-circle" style="width:32px;height:32px;font-size:12px;flex-shrink:0;">${c.name.charAt(0).toUpperCase()}</div><div class="ann-inline-comment-body"><span class="ann-inline-comment-author">${c.name}</span><span class="ann-inline-comment-date">${c.created_at}</span><div class="ann-inline-comment-text">${c.message}</div></div>`;body.appendChild(div);});}
            setTimeout(()=>document.getElementById('asgnModalInput').focus(),80);
        });
    }
    function submitAsgnComment(){
        const input=document.getElementById('asgnModalInput'),msg=input.value.trim(); if(!msg||!currentAsgnId) return;
        ...
        .then(r=>r.json()).then(data=>{
            if(data.success){input.value='';document.getElementById('asgnModalSend').disabled=true;document.getElementById('asgnModalSendImg').style.opacity='0.4';
            const body=document.getElementById('asgnModalBody'),noMsg=body.querySelector('p');if(noMsg)noMsg.remove();
            const div=document.createElement('div');div.className='ann-inline-comment-item';div.innerHTML=`<div class="ann-avatar-circle" style="width:32px;height:32px;font-size:12px;flex-shrink:0;"><?php echo strtoupper(substr($user['name'],0,1)); ?></div><div class="ann-inline-comment-body"><span class="ann-inline-comment-author"><?php echo htmlspecialchars($user['name']); ?></span><span class="ann-inline-comment-date">Just now</span><div class="ann-inline-comment-text">${msg}</div></div>`;body.appendChild(div);}
        });
    }
    ...
    function toggleDotMenu(e,type,id,role){
        ...
        if(type==='ann'){menu.innerHTML=role==='teacher'?`<div class="dot-menu-item" onclick="deleteAnn(${id})">Delete</div><div class="dot-menu-item danger" onclick="reportAbuse('ann',${id})">Report abuse</div>`:`<div class="dot-menu-item danger" onclick="reportAbuse('ann',${id})">Report abuse</div>`;}
        else{menu.innerHTML=role==='teacher'?`<div class="dot-menu-item" onclick="deleteAsgn(${id})">Delete</div><div class="dot-menu-item danger" onclick="reportAbuse('asgn',${id})">Report abuse</div>`:`<div class="dot-menu-item danger" onclick="reportAbuse('asgn',${id})">Report abuse</div>`;}
        document.body.appendChild(menu);activeDotMenu=menu;
    }
    function deleteAnn(id){fetch('delete_announcement.php',{method:'POST',headers:{'Content-Type':'application/x-www-form-urlencoded'},body:'announcement_id='+id}).then(r=>r.json()).then(data=>{if(data.success){if(activeDotMenu){activeDotMenu.remove();activeDotMenu=null;}const card=document.querySelector(`[data-ann-id="${id}"]`);if(card)card.remove();}else alert('Could not delete.');});}
    function deleteAsgn(id){fetch('../assignments/delete_assignment.php',{method:'POST',headers:{'Content-Type':'application/x-www-form-urlencoded'},body:'assignment_id='+id}).then(r=>r.json()).then(data=>{if(data.success){if(activeDotMenu){activeDotMenu.remove();activeDotMenu=null;}const card=document.querySelector(`[data-asgn-id="${id}"]`);if(card)card.remove();}else alert('Could not delete.');});}
