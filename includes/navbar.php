  ...
  <div class="navbar-actions" style="display: flex; gap: 16px; align-items: center;">
    ...
    <div class="theme-switch" onclick="toggleTheme()" aria-label="Toggle dark mode" title="Toggle dark mode">
      <div class="switch-track">
        <div class="track-icon sun"><img src="<?php echo BASE_URL; ?>/icons/brightness-1%20icon%20sun.svg" alt="Sun"></div>
        <div class="track-icon moon"><img src="<?php echo BASE_URL; ?>/icons/brightness-2%20icon%20moon.png" alt="Moon"></div>
      </div>
      <div class="switch-thumb">
        <div class="thumb-icon sun"><img src="<?php echo BASE_URL; ?>/icons/brightness-1%20icon%20sun.svg" alt="Sun"></div>
        <div class="thumb-icon moon"><img src="<?php echo BASE_URL; ?>/icons/brightness-2%20icon%20moon.png" alt="Moon"></div>
      </div>
    </div>
      ...
      <div class="profile-dropdown" id="profileDropdown">
        ...
        <div class="pop-actions-container">
          <a href="#" class="pop-action-btn" onclick="return false;">
            <img src="<?php echo BASE_URL; ?>/icons/circle-plus-icon.svg" class="pop-action-icon" alt="Add"> Add account
          </a>
          <a href="<?php echo BASE_URL; ?>/auth/logout.php" class="pop-action-btn">
            <img src="<?php echo BASE_URL; ?>/icons/Signout.png" class="pop-action-icon" alt="Sign out"> Sign out
          </a>
        </div>
        <div class="pop-footer-links">
          <a href="#" onclick="return false;">Privacy Policy</a> &middot;
...
<div id="studentClassModal" class="student-class-modal" style="display: none;">
  <div class="modal-content">
    ...
    <div id="modalJoinState" style="display: none;">
      ...
      <div class="modal-form-group" id="joinFormGroup" style="position:relative; margin-bottom:8px; text-align:left;">
        <label for="modal_class_code" class="modal-label" id="joinLabel">Class code</label>
        <input type="text" id="modal_class_code" name="code" autocomplete="off" class="modal-input" id="joinCodeInput"
               oninput="clearJoinError()" maxlength="10">
      </div>
 
      <!-- inline error -->
      <div id="joinErrorMsg" style="display:none; color:#d93025; font-size:13px; margin-bottom:16px; text-align:left; padding-left:2px;"></div>
 
      <div style="text-align:right; display:flex; justify-content:flex-end; gap:12px; align-items:center; margin-top:16px;">
        <button type="button" class="modal-cancel-btn" onclick="closeStudentModal()">Cancel</button>
        <button type="button" class="action-btn-join-pill" style="padding:10px 24px;" onclick="submitJoinClass()">Join</button>
      </div>
    </div>
  </div>
</div>
...
.modal-form-group .modal-input {
  ...
  color: var(--text); font-size: 16px; border-radius: 0;
  box-sizing: border-box; outline: none; transition: border-color 0.2s;
}
.modal-form-group .modal-input:focus { border-bottom-color: #1a73e8; }
.modal-form-group.has-focus .modal-label,
.modal-form-group.has-value .modal-label { transform: translate(-10px, -18px) scale(0.75); color: #1a73e8; }
 
/* ── Error state ── */
.modal-form-group.has-error .modal-input { border-bottom-color: #d93025; }
.modal-form-group.has-error .modal-label,
.modal-form-group.has-error.has-focus .modal-label,
.modal-form-group.has-error.has-value .modal-label { color: #d93025; }
...
[data-theme="dark"] .profile-dropdown { background: #202125; box-shadow: 0 4px 20px rgba(0,0,0,0.4); border: 1px solid #444; }
...
.pop-manage-btn { display: block; text-align: center; border: 1px solid #dadce0; border-radius: 50px; background: transparent; color: #1a73e8; font-size: 14px; font-weight: 500; padding: 10px 16px; margin-bottom: 20px; text-decoration: none; transition: background-color 0.2s; }
...
.pop-action-btn { flex: 1; padding: 10px 8px; text-align: center; color: #202124; font-size: 14px; font-weight: 500; text-decoration: none; display: flex; align-items: center; justify-content: center; gap: 6px; background: #ffffff; border: 1px solid #dadce0; border-radius: 50px; transition: background-color 0.2s; box-sizing: border-box; }
...
[data-theme="dark"] .pop-footer-links, [data-theme="dark"] .pop-footer-links a { color: #aaa; }
...
.help-fab {
  ...
  justify-content: center; z-index: 1100; transition: box-shadow 0.2s, background 0.2s; text-decoration: none;
}
...
document.addEventListener('click', function(event) {
  if (!event.target.closest('.plus-container')) document.getElementById('plusDropdown')?.classList.remove('show');
  if (!event.target.closest('.profile-container')) document.getElementById('profileDropdown')?.classList.remove('show');
});
...
function openStudentModal(mode) {
  ...
  clearJoinError();
  if (mode === 'create') {
    document.getElementById('modalCreateState').style.display = 'block';
    document.getElementById('modalJoinState').style.display = 'none';
  } else {
...
function closeStudentModal() {
  const modal = document.getElementById('studentClassModal');
  if (modal) { modal.style.display = 'none'; clearJoinError(); document.getElementById('modal_class_code').value = ''; }
}
...
function openTeacherJoinModal() {
  document.getElementById('plusDropdown')?.classList.remove('show');
  document.getElementById('teacherJoinModal').style.display = 'flex';
}
function closeTeacherJoinModal() {
  document.getElementById('teacherJoinModal').style.display = 'none';
}
 
// ── Join via AJAX ──
function submitJoinClass() {
  const input = document.getElementById('modal_class_code');
  const code = input.value.trim();
  if (!code) { showJoinError('Please enter a class code.'); return; }
 
  fetch('<?php echo BASE_URL; ?>/classes/join.php', {
    method: 'POST',
    headers: {
  ...
  .catch(() => showJoinError('Something went wrong. Please try again.'));
}
 
function showJoinError(msg) {
  const group = document.getElementById('joinFormGroup');
  const errEl = document.getElementById('joinErrorMsg');
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (codeInput) {
    codeInput.addEventListener('keydown', e => { if (e.key === 'Enter') submitJoinClass(); });
